...
void SX1276LoRaInit( void )
{
  ...
  printf("SX1276ReadBuffer REG_LR_OPMODE %d\n", *(uint8_t *)(SX1276Regs + 1));
...
void SX1276LoRaReset( void )
{
  ...
  usleep(1000);//while( ( GET_TICK_COUNT( ) - startTick ) < TICK_RATE_MS( 1 ) );
...
void SX1276LoRaGetPacketRssi(double* Rssi )
{
  *Rssi= RxPacketRssiValue;
}

void SX1276LoRaSetPacketRssi(double Rssi )
{
  RxPacketRssiValue=Rssi;
}
  ...
  case RFLR_STATE_RX_RUNNING:
	{
	  ...
	  if( LoRaSettings.RxSingleOn == true ) // Rx single mode
	  {
		if(((uint32_t)(GET_TICK_COUNT()-(RxTimeoutTimer))) > PacketTimeout )
		{
  ...
  case RFLR_STATE_TX_RUNNING:
	{
	  ...
	  //TxTimeoutTimer++;
	  if(((uint32_t)(GET_TICK_COUNT()-(TxTimeoutTimer))) > PacketTimeout )
	  {
...
uint32_t GetRsSpeed(void)
{
  const uint32_t SignalBwTab[]={7800,10400,15600,20800,31200,41600,62500,125000,250000,500000};
  // SignalBw [0: 7.8kHz, 1: 10.4 kHz, 2: 15.6 kHz, 3: 20.8 kHz, 4: 31.2 kHz,
  // 5: 41.6 kHz, 6: 62.5 kHz, 7: 125 kHz, 8: 250 kHz, 9: 500 kHz
  return SignalBwTab[LoRaSettings.SignalBw]>>(LoRaSettings.SpreadingFactor);
}


void SetTxTimeout(uint32_t preamble_size)
{
  LoRaSettings.TxPacketTimeout=(preamble_size+4+8+LoRaSettings.PayloadLength+2)*1000/(GetRsSpeed());
  LoRaSettings.TxPacketTimeout+=(LoRaSettings.TxPacketTimeout>>1);
}

void SetRxTimeout(uint32_t preamble_size)
{
  LoRaSettings.RxPacketTimeout=(preamble_size+4+8+LoRaSettings.PayloadLength+2)*1000/(GetRsSpeed());
  LoRaSettings.RxPacketTimeout+=(LoRaSettings.TxPacketTimeout>>1);
}
